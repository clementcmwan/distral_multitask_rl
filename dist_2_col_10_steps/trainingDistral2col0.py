...
def trainD(file_name="Distral_1col", list_of_envs=[GridworldEnv(5),
    ...
    while np.min(episodes_done) < num_episodes:
        ...
        for i_env, env in enumerate(list_of_envs):
            ...
            # or len(memories[i_env]) >= max_num_steps_per_episode
        ...
        # for i_env in range(num_envs):
            # if len(memories[i_env]) >= max_num_steps_per_episode:
                # memories[i_env].clear_memory()
                # done_flags[i_env] = 0
...
if __name__ == '__main__':
    trainD(list_of_envs=[GridworldEnv(4),GridworldEnv(5),GridworldEnv(6),GridworldEnv(7),GridworldEnv(8)], learning_rate=0.001, max_num_steps_per_episode=100, num_episodes=1000, alpha=1.)
