...
def task_specific_update(policy, distilled, opt_policy, alpha, beta, gamma, final_state_value, task_id):
    ...
    sample_size = min(100, torch.stack(task_policy_loss).size()[0])
    sample_indx = np.random.choice(torch.stack(task_policy_loss).size()[0], sample_size)
    sampled_task_loss = torch.stack(task_policy_loss)[sample_indx]
    sampled_critic_loss = F.mse_loss(q_values[sample_indx], v_thetas[sample_indx].unsqueeze(1))

    # loss = -(torch.stack(task_policy_loss).mean()) + critic_loss

    loss = -(sampled_task_loss.mean()) + sampled_critic_loss
...
def finish_episode(task_specific_loss, policies, distilled, opt_distilled, alpha, beta, gamma):
    ...
    sample_size = min(100, torch.stack(mismatch_loss).size()[0])
    sample_indx = np.random.choice(torch.stack(mismatch_loss).size()[0], sample_size)
    sampled_mismatch_loss = torch.stack(mismatch_loss)[sample_indx]


    loss =  -(torch.stack(task_specific_loss).sum() + (alpha/beta) * sampled_mismatch_loss.sum())


    # loss =  -(torch.stack(task_specific_loss).sum() + (alpha/beta) * torch.stack(mismatch_loss).sum())
    # print(torch.stack(mismatch_loss).sum())
...
if __name__ == '__main__':
    ...
    trainDistral(list_of_envs=[GridworldEnv(4), GridworldEnv(5)], learning_rate=0.0001, num_episodes=200, beta=5)
