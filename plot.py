...
print(np.load('new_exp/dist_1_col_AC_2_betas/Distral_1col-distral0-durations.npy').shape)

distral4_rewards = np.load('new_exp/dist_1_col_AC_2_betas/Distral_1col-distral0-durations.npy')[:,0]
distral5_rewards = np.load('new_exp/dist_1_col_AC_2_betas/Distral_1col-distral0-durations.npy')[:,1]

distral4_smooth = pd.Series(distral4_rewards).rolling(smoothing_window,min_periods=2).mean()
distral5_smooth = pd.Series(distral5_rewards).rolling(smoothing_window,min_periods=2).mean()

plt.figure(figsize=(10,5))
plt.title('Distral 2 col vs SQL over training tasks', fontsize='20')
